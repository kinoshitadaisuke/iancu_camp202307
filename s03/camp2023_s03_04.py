...
#
# Time-stamp: <2023/07/28 10:33:12 (CST) daisuke>
#
...
sql_maketable = f'create table bsc5 (hr integer primary key, name text, ' \
...
with contextlib.closing (sqlite3.connect (file_db)) as conn:
    ...
    with gzip.open (file_catalogue, 'rb') as fh_bsc:
        ...
        for line in fh_bsc:
            ...
            sql_adddata = f'insert into bsc5 values ({HR}, "{name}", ' \
                + f'"{RA_str}", {RA_deg}, "{Dec_str}", {Dec_deg}, ' \
                + f'{glon}, {glat}, {mag_V}, {colour_BV}, ' \
                + f'"{sptype}", {pm_RA}, {pm_Dec}, {parallax});'

            # executing SQL command to add star's data to database
